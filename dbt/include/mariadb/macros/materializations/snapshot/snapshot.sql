...
{# need to override because database=none #}
{% materialization snapshot, adapter='mariadb' %}
  ...
  {% else %}
      ...
      {% set final_sql = mariadb__snapshot_merge_sql_insert(
            target = target_relation,
            source = staging_table,
            insert_cols = quoted_source_columns
         )
...
{% macro snapshot_staging_table_USE_DEFAULT(strategy, source_sql, target_relation) -%}
